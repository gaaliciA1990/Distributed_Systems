...
"""
THIS SECTION IS THE CONNECT TO BITCOIN CODE
"""
...
def connect_to_btc(block_number: int):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as btc_sock:
        ...
        block_hash = swap_endian(GENESIS)
        curr_height = 0
...
def send_version_message(btc_sock: socket) -> int:
    ...
    return unmarshal_uint(peer_version_msg[-5:-1])
...
def send_ping_message(btc_sock):
    ...
    ping_msg = get_message('ping', uint64_t(random.getrandbits(64)))
...
def version_message() -> bytes:
    ...
    """
    version = int32_t(VERSION_NUM)  # version = 700015
    services = uint64_t(0)  # Not a full node
    ...
    addr_trans_ip_address = ipv6_from_ipv4(HOST_IP)
...
def exchange_messages(send_msg: bytes, btc_sock: socket, expected_bytes=None, height=None, wait=False) -> list:
    ...
    """
    print_message(send_msg, 'send', height)
    recvd_bytes = b''
    address = (BTC_IP, PORT)
    ...
    try:
        ...
        for message in message_list:
            print_message(message, 'receive', height)
...
def split_msg(peer_message: bytes) -> list:
    ...
    while peer_message:
        payload_size = unmarshal_uint(peer_message[16:20])
...
"""
THIS SECTION IS THE INTERPRETER, RESPONSIBLE FOR PRINTING AND (DE)SERIALIZING THE MESSAGES
"""
